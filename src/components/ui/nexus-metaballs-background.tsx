...
export interface NexusMetaballsProps {
  className?: string;
  preset?: "holographic" | "cosmic" | "neon" | "midnight" | "moody";
}
...
const PRESETS = {
  holographic: {
    ...
    specularPower: 4,
    fresnelPower: 0.9,
    backgroundColor: new THREE.Color(0x000208),
    sphereColor: new THREE.Color(0x020818),
    lightColor: new THREE.Color(0x60a5fa),
...
export const NexusMetaballsBackground: React.FC<NexusMetaballsProps> = ({
  className = "",
  preset = "holographic",
}) => {
  ...
  useEffect(() => {
    ...
    const currentPreset = PRESETS[preset] || PRESETS.holographic;
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uSphereCount: { value: isMobile ? 4 : currentPreset.sphereCount },
        ...
        uSmoothness: { value: currentPreset.smoothness },
      ...
      fragmentShader: `
        ...
        float sceneSDF(vec3 pos) {
          ...
          int maxIter = uIsMobile > 0.5 ? 4 : min(uSphereCount, 8);
          for (int i = 0; i < 8; i++) {
            if (i >= uSphereCount || i >= maxIter) break;
            
            float fi = float(i);
            float speed = 0.4 + fi * 0.12;
            float radius = 0.14 + mod(fi, 3.0) * 0.06;
        ...
        float rayMarch(vec3 ro, vec3 rd) {
          float t = 0.0;
          int maxSteps = uIsMobile > 0.5 ? 24 : 42;
        ...
        vec3 lighting(vec3 p, vec3 rd, float t) {
          ...
          vec3 fresnelRim = uLightColor * fresnel * 0.45;
    ...
    const screenToWorldJS = (normX: number, normY: number) => {
    ...
    const onPointerMove = (event: PointerEvent | MouseEvent | Touch) => {
      const rect = container.getBoundingClientRect();
      targetMousePosition.x = (event.clientX - rect.left) / width;
      targetMousePosition.y = 1.0 - (event.clientY - rect.top) / height;
    ...
    const onResize = () => {
      ...
      const curPixelRatio = Math.min(window.devicePixelRatio || 1, isMobile ? 1.5 : 2);

      renderer.setSize(width, height);
      renderer.setPixelRatio(curPixelRatio);

      material.uniforms.uResolution.value.set(width, height);
      material.uniforms.uActualResolution.value.set(width * curPixelRatio, height * curPixelRatio);
      material.uniforms.uPixelRatio.value = curPixelRatio;
    };
...
export default NexusMetaballsBackground;
