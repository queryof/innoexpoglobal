...
export const WhatWeDo = () => {
  ...
  return (
    <section id="what-we-do" className="relative z-10 w-full border-t border-zinc-200 bg-[#f0f7ff] text-zinc-900">
      ...
      <div className="py-20 sm:py-24 px-6 sm:px-12 max-w-4xl text-left">
        <AppleReveal>
          ...
          <h2 className="text-3xl sm:text-5xl font-serif text-zinc-950 tracking-tight leading-[1.15]">
            ...
            <span className="font-serif italic font-light text-blue-600">the global stage</span>
          </h2>
      ...
      {/* 3x2 Grid of Activities (Light Theme) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border-t border-zinc-200 divide-y lg:divide-y-0 lg:divide-x divide-zinc-200">
        {activities.slice(0, 3).map((act, i) => {
          ...
          return (
            <div
              key={i}
              className="p-8 sm:p-10 transition-colors duration-200 hover:bg-zinc-50/70 flex flex-col justify-between text-left min-h-[260px]"
            >
              <AppleReveal delay={i * 0.1}>
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-xs font-mono text-zinc-400">{act.num}</span>
                  </div>
      ...
      <div className="border-t border-zinc-200 grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x divide-zinc-200 items-center bg-zinc-50/30">
        <div className="lg:col-span-8 p-8 sm:p-10 text-left">
          <AppleReveal>
            <div className="flex items-center gap-2 text-blue-600 mb-2">
              <Cpu className="w-4 h-4" />
              <span className="text-xs font-mono uppercase text-zinc-500">Activity 07</span>
            </div>
            <h3 className="text-xl sm:text-2xl font-serif text-zinc-950 mb-2">
        ...
        <div className="lg:col-span-4 p-8 sm:p-10 flex items-center justify-start lg:justify-center">
          <AppleReveal delay={0.15}>
            <Link
              href="/register"
              className="inline-flex items-center gap-2 rounded-xl bg-zinc-950 hover:bg-zinc-800 text-white backdrop-blur-md px-6 py-3 text-xs sm:text-sm font-medium transition-all active:scale-95 shadow-md shadow-zinc-900/10"
            >
      ...
      <div className="border-t border-zinc-200 p-8 sm:p-12 text-left bg-zinc-50/70">
        <AppleReveal>
          <div className="flex items-center gap-2 text-blue-600 mb-2">
          ...
          {/* Sequential Step Progression (Light Theme) */}
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {impactSteps.map((item, idx) => (
              <div
                key={idx}
                className="rounded-xl border border-zinc-200 bg-white p-4 text-left flex flex-col justify-between relative group hover:border-blue-400 hover:shadow-sm transition-all"
              >
                <div className="text-[10px] font-mono text-zinc-400 mb-1">
                  Step {item.label}
                </div>
                <div className="text-sm font-serif text-zinc-950 font-medium flex items-center justify-between">
                  <span>{item.step}</span>
                  {idx < impactSteps.length - 1 && (
                    <ChevronRight className="w-3.5 h-3.5 text-zinc-400 group-hover:text-blue-600 transition-colors hidden lg:block" />
                  )}
                </div>
              </div>
            ))}
